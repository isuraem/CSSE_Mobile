...
const Signuplocal = ({navigation}) => {
  ...
  const handleSubmit = async () => {
    ...
    if(!Name|| !Email || !Phone || !NIC || !Password || !Address) {
    ...
    try{
      ...
      const {data}= await axios.post("http://192.168.1.23:8070/traveller/addLocalT",{
        Name,
        Email,
        Phone,
        NIC,
              ...
              <View style = {{ flex:1,flexDirection:"row",marginTop:10,justifyContent:'center'}}>
                  <TouchableOpacity disabled={true}
                        style = {{
                            ...
                            <Text bold medium center style = {{ color: 'white',fontSize:15,}} onPress = { () => navigation.navigate("Signup")}>
                            Foreign
                            </Text>
                  </TouchableOpacity>
